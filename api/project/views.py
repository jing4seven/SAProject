...
from lib.wrapper import condition
...
class projects_view(generics.GenericAPIView,
    ...
    def get_projects_etag(request, *args, **kwargs):
        ct = project.objects.count()
        latest_row = project.objects.raw('SELECT id, update_time FROM sa_t_projects ORDER BY update_time DESC LIMIT 1')
        return hashlib.md5("%s$%s" % (ct, latest_row[0].update_time)).hexdigest()

    def get_projects_last_modified(reuqest, *args, **kwargs):
        latest_row = project.objects.raw('SELECT id, update_time FROM sa_t_projects ORDER BY update_time DESC LIMIT 1')
        return latest_row[0].update_time

    @condition(etag_func=get_projects_etag, last_modified_func=get_projects_last_modified)
